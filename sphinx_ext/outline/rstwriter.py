...
class RstWriter(writers.Writer):
    ...
    def __init__(self, builder):
        ...
        #fout = open("outline.rst","w")
        #fout.write("Course Outline\n##############\n\n")
        #fout.close()
...
class RstTranslator(TextTranslator):
    ...
    def log_unknown(self, type, node):
        ...
        if len(logger.handlers) == 0:
            ...
            logging.basicConfig(filename="outline.log", level=logging.INFO, format='%(levelname)-8s %(message)s')
            logger = logging.getLogger("sphinxcontrib.writers.rst")
    ...
    def visit_objective(self, node):
        logger.warning("%s(%s) objective" % (type, node))
    ...
    def depart_title(self, node):
        ...
        fout = open("outline.rst","a")
        fout.write("%d:%s\n" % (self.sectionlevel, text))
        fout.close()
    ...
    def unknown_visit(self, node):
        print('not implemented %s' % node.__class__.__name__)
