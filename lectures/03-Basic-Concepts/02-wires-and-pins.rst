...
wires and pins. Let's look more closely at these things now.

Pins
****

A pin is just an attachment point on the side of our component. We will connect
a wire to a pin and a signal of some sort will either arrive at that pin, or be
generated by the part owning the pin, and that signal will move to some other
place.
...
It will prove handy for us to model a pin as a container with a storage place
...
on a part). But a single wire can be attached to to the pins on multiple other
parts. The attachments are always to pins set up to receive an input signal. If
we tried to hook up two output pins to a single wire, the result would be a
mess.  Whatever data we were trying to move over the wire would be garbled.
...
We need three basic classes:
   ...
   * A generic component class modeling some component with a set of pins
     connected to it. The exact number of pins depends on the gadget.
...
The "when" part of this is actually the heart of our simulation. We need to talk
about time, and how it figures into our work!
